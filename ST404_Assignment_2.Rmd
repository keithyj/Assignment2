...
```{r, include = FALSE}
#Loading the data
load(paste(getwd(), "cancer.rdata", sep="/"))
attach(cancer)
#Storing raw data
cancerRaw <- cancer
#Loading packages
...
cancer <- cancer %>% filter(incidenceRate!=1014.2)  
```
...
This model contains 11 regressor variables (including the region factor variable which has 4 levels) which is a lot of variables and would make it hard to interpret the model. Predictive power will be good but the complexity needs to be penalised more, so we will use the BIC instead.
...
This produces a model with 8 variables (again including the region factor variable) which is a lot more manageable and easy to interpret, it has a standard residual error that is only 0.0005 greater than the model that uses AIC. So it fits the data nearly as well, but there are far fewer variables. This model can even be further simplified because when the full output of the stepwise regression function is printed, the change in the value of the BIC is very little in the last steps compared to the first steps. We can simplify our model further by removing `PctBlack`, `PctPrivateCoverage`, and `PctPublicCoverage`. This reduces our R-squared value by 0.015 and our adjusted R-squared value by 0.0147 but it also removes three variables which makes the model simpler. By only including one coverage variable, we also see reduced VIF values. With all coverage variables VIF values of over 6 were present. PctEmpPrivCoverage was chosen since it has the lowest p-value in the above model and also shows slightly smaller signs of collinearity with the non-coverage regressor variables. All p-values for this model are below 0.0002, which is very low.

(??) Perhaps the part about removing the coverage variables could be moved to the multicollinearity section but not sure as the summary tables take up quite a lot of pages (??).

```{r}
altBICTransModel=lm(deathRate~incidenceRate + medIncome + Region +
PctUnemployed16_Over + Edu18_24 + PctEmpPrivCoverage, data=cancer2)
...
altBICTransModel=lm(deathRate~incidenceRate + povertyPercent + Region +
PctUnemployed16_Over + Edu18_24 + PctEmpPrivCoverage, data=cancer2)
summary(altBICTransModel)
ols_coll_diag(altBICTransModel)
extractAIC(altBICTransModel,k=log(3047))
#Remove intercept term
altBICTransModel=lm(deathRate~ 0 + incidenceRate + povertyPercent + Region +
PctUnemployed16_Over + Edu18_24 + PctEmpPrivCoverage, data=cancer2)
summary(altBICTransModel)
ols_coll_diag(altBICTransModel)
extractAIC(altBICTransModel,k=log(3047))
#Remove Edu18_24
altBICTransModel=lm(deathRate~ 0 + incidenceRate + povertyPercent + Region +
PctUnemployed16_Over + PctEmpPrivCoverage, data=cancer2)
summary(altBICTransModel)
ols_coll_diag(altBICTransModel)
extractAIC(altBICTransModel,k=log(3047))
```
...
```{r histogramAvgHouseholdSize included, echo = FALSE, eval = TRUE, message = FALSE, warning = FALSE, fig.asp = 0.6, fig.cap = "Histograms of AvgHouseholdSize after scaling small values"}
par(mfrow = c(1,2))
