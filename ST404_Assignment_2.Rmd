...
modelTable=as.table(matrix(data=c(beta),nrow=1,ncol=9))
colnames(modelTable)=name
rownames(modelTable)=c('Parameter estimate')
pander::pander(modelTable, caption = "Information on LASSO Model", split.cell = 75, split.table = Inf, style = "grid", justify = c("left","center","left","left","left","left","left","left","left","left"), use.hyphening = TRUE)
```


```{r Cross Validation LASSO, echo = FALSE} 
par(mfrow = c(1,1))
