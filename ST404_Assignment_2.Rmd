...
What to still mention:

* The multicollinearity shown in statistical methodology

We noticed that medIncome and binnedInc represent the same force and so we removed binnedInc from the data before we conducted any analysis.  
...
```{r, echo=FALSE}
dfBetaVals=dfbetas(Model1)
indices=which(abs(dfbetas(Model1))>0.4)
outliers=(which(abs(dfbetas(Model1))>0.4)%%3047)
names=unique(cancer$County[outliers])
tableOutliers=unique(outliers)
...
```
We add a variable describing the region in which observation the county is in. We split the US into 4 regions defined by the Census Bureau-designated regions and divisions (United States Census Bureau). These regions are Northeast, Midwest, South and West.
