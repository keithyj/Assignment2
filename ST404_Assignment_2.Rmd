...
summary(BICmodel4)
```
When completing stepwise regression, we found that there were issues with collinearity with the condition number was too high. We aimed to reduce it by removing variables that minimised that increase in the BIC value while aiming for a condtion number below 30, using our initial ideas to inform the changes. The table below shows how the model was altered at each change.
...
We looked into ridge regression to help deal with any multicolineaity issues that may arise. Using this form of peanalised regresssion gives us a model with an R squared of 0.484 and a mean squared error of 378.5, which is low since ridge regression trades bias for a reduced variance. Given that the R squared isn't great in comparison to the model made using LASSO we opt to use other peanalilsed regression methods for greater simplicity and explanatory power.
