...
\tiny
```{r Model Table Step, echo=FALSE,message=FALSE,warning=FALSE}
library(sylly)
name=c('incidenceRate','povertyPercent','PctUnemployed16_ Over',
       'PctPublicCoverage','PctBlack','PercentMarried')
transformation=c('(incidenceRate^(1.5)-1)/1.5','None','(PctUnemployed16_Over^(0.5)-1)/0.5','None','log(PctBlack+0.1)',
                'None')
beta=c('0.00768','1.53','4.71','0.556','2.02','0.858')
interpretation=c("If incidence rate increases by a factor of x, death rate increases by a factor of x^1.5 with a small error due to the boxcox transform, assuming all other variables are zero. If incidence rate^1.5 increases by 1, death rate increases by 0.00512 assuming all other variables remain constant.", "If poverty percent increases by 1, death rate increases by 1.53, assuming all other variables remain constant.", "If percent unemployed increases by a factor of x, death rate increases by a factor of x^0.5 with a small error due to the boxcox transform, assuming all other variables are zero. If percent unemplyed^0.5 increases by 1, death rate increases by 9.42 assuming all other variables remain constant.", "If percent public coverage increases by 1, death rate increases by 0.556, assuming all other variables remain constant.", "If (percent black + 0.1) increases by a factor of x, death rate will increase by 2.02log(x), assuming all other variables remain constant.", "If percent married increases by 1, death rate increases by 0.858, assuming all other variables remain constant.")
...
\normalsize
There were issues with collinearity using this method, meaning lots of variables were removed. The model suggested by stepwise regression is shown above.
