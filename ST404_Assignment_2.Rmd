...
```
Cook's distance, using all predictor variables, shows us that incidence rate has high influence and so gives us sufficient evidence to remove the outlier in incidenceRate.
``` {r removeWilliamsburg, include = FALSE}
cancer <- filter(cancer, incidenceRate <= 900) 
```
