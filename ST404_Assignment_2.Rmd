...
Some transformations of the variables were completed to fulfil the model assumptions, mainly homoscedasticity (constant variance of errors) and linearity. Transformations were kept to a minimum to keep the model as simple as possible. One of our transformations required us to first scale PctBlack by 0.1. Hence, the transformations we use for building a model are boxcox transformations os incidenceRate and PctUnemployed16_Over as follows (incidenceRate^1.5-1)/1.5 and (sqrt(PctUnemployed16_Over)-1)/0.5, also log transform PctBlack and medIncome.
...
Conformity <- function(x){ #Calculating residuals for one county, using raw dataset with PctEmployed16_Over removed
  return(cancer[x,17]-(387+0.01*boxcox(cancer[x,3],1.5)-23.5*log(cancer[x,4])+2.49*boxcox(cancer[x,10],0.5)-0.212*cancer[x,11]+0.127*cancer[x,13]-8.22*cancer[x,16]-4.61*(cancer[x,18]=="NorthEast")+3.73*(cancer[x,18]=="South")-12.4*(cancer[x,18]=="West")))#Calculating residuals
}
