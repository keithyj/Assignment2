...
``` {r adequacy, echo = FALSE, eval = TRUE}
...
``` {r transformed variables, include = FALSE}
cancer5=cancer
...
```
We have 3040 observations in our sample space, so this is large. What does this mean?
...
``` {r multicolliearity2, echo = FALSE}
# Look at scatter plots and compare weak positive correlations to the correlations shown in original plots.
par(mfrow = c(2,3))
with(cancer5, scatter.smooth(deathRate~Edu18_24, ylab = "deathRate", xlab = "Edu18_24", lpars = list(col = "red", lwd = 2), main = "deathRate against Edu18_24", cex.main = 1))
with(cancer5, scatter.smooth(deathRate~PctMarriedHouseholds, ylab = "deathRate", xlab = "PctMarriedHouseholds", lpars = list(col = "red", lwd = 2), main = "deathRate against PctMarriedHouseholds", cex.main = 1))
...
```{r Clean and Transform, include=FALSE}
cancer$AvgHouseholdSize[cancer$AvgHouseholdSize <= 0.1] <- cancer$AvgHouseholdSize[cancer$AvgHouseholdSize <= 0.1]*100
cancer <- subset(cancer, select=-c(PctEmployed16_Over,binnedInc))
cancer5=cancer
cancer5$incidenceRate=(cancer5$incidenceRate)^1.5
cancer5$medIncome=log(cancer5$medIncome)
cancer5$PctBlack=log(cancer5$PctBlack+0.1)
cancer5$PctUnemployed16_Over=cancer5$PctUnemployed16_Over^(1/2)
cancer5 <-  slice(cancer5, -c(2714, 2628, 166, 1059, 1942, 1094, 282))
...
The first thing we do is run a LASSO on our data (including region and with transformations on percent black and incidence rate). From this we use cross validation to find an optimal value of lambda. From our cross validation we see that using the value of lambda minimising mean squared error will include almost all our variables. Therefore we opt to use the value of lambda 1se away from this minal value. The model generated with this value of lambda includes the followign variables: incidence rate, median income, poverty percentage, percent unemployed, percent private coverage, percent public coverage, education score and region. 
This model has an R squared value of 0.4678. In addition, both linearity and homoscedascity are satisfied alothough there is a slight violaiton in normality of residuals. All coefficents of variables have the same sign as their correlation with death rate.
We then attempted to extend this model using quadratic terms and interaction terms between highly correlated variables. However the interaction terms were not being included and the quadratic terms whcih were included would shrink their linear counterparts to zero. Hence we leave these extensions out.
