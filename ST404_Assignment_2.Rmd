...
```
The variables we will include in the initial model are those that show a relationship with deathRate. From the scatter plots we can observe that most variables show some type of relationship with deathRate. However, it is not clear to see whether any do not. For now, we will assume all predictor variables do and keep them in the model, especially as we have not yet tested for multicollinearity which could affect the model. We will soon do an analysis of variance to check the significance of the relationships between predictors and deathRate.
...
``` {r checkingassumptions, echo = FALSE, eval = TRUE,out.height='80%',out.width='80%', fig.cap = "Showing the improved linearity and homoscedasticity of PctBlack and PctUnemployed16_Over"}
par(mar=c(1.8, 4.4, 2, 1.8))
par(mfrow=c(2,2))
plot(lm(deathRate ~ PctBlack, data = cancer5), 1, cex=0.5,main="log(PctBlack) Residuals Fitted")
plot(lm(deathRate ~ PctBlack, data = cancer5), 3, cex=0.5,main="log(PctBlack) Scale location")
plot(lm(deathRate ~ PctUnemployed16_Over, data = cancer5), 1, cex=0.5,main="PctUnemployed16^0.5 Residuals Fitted")
plot(lm(deathRate ~ PctUnemployed16_Over, data = cancer5), 3, cex=0.5,main="PctUnemployed16^0.5 Scale location")
```

These transformations show evidence of homoscedascity and linearity thus we will update the new model with the transformed variables. The two transformed variables that are not shown here are in the appendix. After transforming the varibales, we check the model assumptions for this updated model and notice the model assumptions are being violated, this could be due to influencial outliers.
