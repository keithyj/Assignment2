...
We tried to limit the number of transformation we used for simplicity. The transformations we used were a box cox transform of incidence rate and percent unemployed with p values 1.5 and 0.5 respectively. Also a log transform of PctBlack + 0.1 and medIncome.

## Outliers and Influential Observations

Several observations were identified as having a high leverage and after testing for influential observations we declared that the following observations would be removed from the data to ensure the model was not impacted by outliers.
...
There were issues with collinearity using this method, meaning variables were removed incrementally until a suitabl moddel was found. The final model found from stepwsie variable selection is presented below.
```{=Latex}
$deathRate=0.08\frac{incidenceRate^{1.5}-1}{1.5}-1.53povertyPercent+4.71\frac{PctUnemployed16\_Over^{0.5}-1}{0.5}+0.556PctPublicCoverage+2.02\log(PctBlack+0.1)+0.858PercentMarried$
```
This model was compared with the LASSO model below, and the LASSO model was preferred due to its superior predictive and explanatory power.

### LASSO regression

\normalsize
Using LASSO variable selection, we create a model which includes eight predictor variables including: those in the table below. We expected this as the scatter plots and summary of our initial model suggested they are significant with respect to death rate except for medIncome. We found this model to have good predictive power. The coefficients for each of these variables are as follows.

\tiny
```{r Model Table LASSO, echo = FALSE, warning=FALSE}
library(sylly)
...
transformation=c('intercept','(incidenceRate^(1.5)-1)/1.5','log(medIncome)','(PctUnemployed16_Over^(0.5)-1)/0.5','None','None','None','None','None','Npne')
interpretation=c("If all other variables were equal to zero death rate would be given as 387, we discus this further below","If incidence rate increases by a factor of x, death rate increases by a factor of x^1.5 with a small error due to the boxcox transform, assuming all other variables are zero. If incidence rate^1.5 increases by 1, death rate increases by 0.00512 assuming all other variables remain constant.", "If median income increases by a factor of x, death rate will increase by a factor of -23.54log(x) assuming all other variables remain constant", "If percent unemployed increases by a factor of x, death rate increases by a factor of x^0.5 with a small error due to the boxcox transform, assuming all other variables are zero. If percent unemplyed^0.5 increases by 1, death rate increases by 9.42 assuming all other variables remain constant.","If percent private coverage increases by 1, death rate will increase by 0.127, assuming all other variables remain constant", "If percent public coverage increases by 1, death rate increases by 0.127, assuming all other variables remain constant.","If education score increases by one, death rate will decrease 8.22, assuming all other variables remain constant", "If a county belongs to a state in the North Ease region then death rate will decrease by 4.61", "If a count belongs to a state in the South region then death rate will increase by 3.73", "If a county belongs to a state in the West region then death rate will decrease by 12.4")       
modelTable=as.table(matrix(data=c(transformation,beta,interpretation),nrow=10,ncol=3))
rownames(modelTable)=name
colnames(modelTable)=c('Transformation','Parameter estimate','Interpretation')
pander::pander(modelTable, caption = "Information on LASSO Model", split.cell = 75, split.table = Inf, style = "grid", justify = c("left","right","right","center"), use.hyphening = TRUE)
```
...
Including all outliers, we find that most states conform to our model, and therefore the general pattern, very well. However, death rates in Alaska are predicted to be 38.7 higher than the actual value on average per county. This is by far the most unconforming state. This may be caused by a lower sample size in terms of both population and number of counties.  All other states have a average prediction within 14 of the actual death rate across all counties and so mostly conform to the general pattern. In terms of individual counties, we find that Willamsburg City, Virginia;  Aleutians West Census, Alaska; North Slope Borough, Alaska, are the states with the highest residuals, with residuals over 90.  
...
Many, 152, PctEmployed16_Over observations are missing. We could fill the missing observations by prediction using a linear model created by the other variables. However, the variable PctUnemployed16_Over is strongly correlated with PctEmployed16_Over. Therefore, we choose to remove PctEmployed16_Over to avoid any issues with missing observations and collinearity, and to avoid increasing the variance of the model by performing further predictions of the missing values. We will also scale the values of AvgHouseholdSize between 0 and 0.1 by 100. We add a variable called "Region" which describes what observation the county is in. We split the US into 4 regions: Northeast, Midwest, South, and West [@Divisions]. Lastly, we choose to ignore binnedInc in our modelling process as it represents the same force as medIncome.
...
``` {r, echo = FALSE, eval = TRUE,fig.height=2,fig.width=6, fig.cap = "Checking for linearity and homoscedasticity of the initial model"}
...
We can see from the plots that there exists heteroscedasticity and non-linearity. We know that some predictor variables are violating model assumptions, so we will need to perform some transformations of some variables to fix these. We do this by using Box-Cox and observations after trialling log transformations. We first transform the predictor variables to satisfy homoscedasticity, these include the following transformations: $log$(PctBlack$+0.1$), boxcox incidenceRate with power 1.5, boxcox PctUnemployed16_Over with power 0.5 and  $log$(medIncome).
...
``` {r checkingassumptions print, include = FALSE}
...
``` {r checkingassumptions, echo = FALSE, eval = TRUE,fig.height = 2, fig.cap = "Showing the improved linearity and homoscedasticity of PctUnemployed16Over"}
...
These transformations show evidence of homoscedasticity and linearity thus we will update the new model with the transformed variables. This is also true for the three transformed variables that are not shown here. After transforming the variables, we check the model assumptions for this updated model and notice the model assumptions are being violated, this could be due to influential outliers - so we will check this.
...
If outliers have high influence, then they will need to be considered for removal. Influential outliers are those that effect the slope of the model. We will calculate if any influential outliers exist by using residual leverage plots and DFBETAS. Any DFBETA values greater than 0.4 and outliers that have high leverage will be removed from the data.
...
```
After removing these influential observations, we have 3041 observations in our sample space, this is a large sample space and so we can pay less attention to the errors being normally distributed as it is not a major concern due to the likelihood of errors being large. We can then check if deathRate highly correlates to any of the explanatory variables by analysing the model's summary statistics and test the null hypothesis that there is no linear association between deathRate and all individual predictor variables. 

``` {r summarymod3, include = FALSE}
anova(Model3)
```

We focus on the relationships between deathRate and medianAgeMale, medianAgeFemale, PercentMarried, PctBlack,and PctMarriedHouseholds. We observe the p-values for these which, respectively, show 0.656,  0.936, 0.360, and 0.141. As the p-value is above 0.05, we fail to reject the null hypothesis for these relationships. All other predictor variables reject the null hypothesis. We do not yet remove any variables from the model as we first want to check any existing redundancies.

We can determine that our response variable has a relationship with at least one of the variables in the model and we can further test which model is optimal by testing for redundancy. We will spot this by comparing the summary output with the scatter plots we saw in our EDA. They show a negative correlation between deathRate and PctMarriedHouseholds, and PctEmpPrivCoverage, which are not negative in the summary of the model. Moreover, the estimate is negative for PovertyPercent and PctBlack in the summary statistics but not in the plot. Also MedianAgeMale and MedianAgeFemale plots against deathRate that do not correlate with the estimate in the summary statistic. This shows there exists redundancy and can be explained by any multicollinearity.
...
summary(BICmodel4)
```
When completing stepwise regression, we found that there were issues with collinearity with the condition number was too high. We aimed to reduce it by removing variables that minimised that increase in the BIC value while aiming for a condition number below 30, using our initial ideas to inform the changes. The table below shows how the model was altered at each change.

\tiny
```{r Change log 1,echo=FALSE}
changes=matrix(0,nrow=4,ncol=3)
BICs=c(18188,18188,18227,18561)
CondNos=c(264,250,63.8,19.2)
Excluded=c('NA','Intercept','medIncome','Region')
changes[,1]=Excluded
changes[,2]=BICs
changes[,3]=CondNos
changes=as.table(changes)
colnames(changes)=c('Removed variable','BIC','Condition Number')
rownames(changes)=paste('Model no.',1:4)
pander::pander(changes, caption = "Change log for the BIC model")
```

\normalsize
The condition number was now below 19 and the VIFs were all below 3.5. Consequently, we decided that we could use this as our model selected by stepwise regression as collinearity was no longer an issue.

**Check Model Assumptions**

The graphs below show that the Residuals vs Fitted plot and the Scale-Location plot look very slightly quadratic which is shown by the red line. However, the residuals appear to be randomly distributed around 0 and there seems to be no correlation between them. There also does not appear to be issues with heteroscedasticity. The errors in the Normal Q-Q plot, in the appendix, appear to fall away from the line in the tails but we have 3041 data points and so the normality in errors does not need to be followed as strictly. Finally, the Cook's Distance, in the appendix, plot shows there are some points which had some influence but there was no justification for omitting them as an outlier.

```{r Check Assumptions, echo=FALSE, fig.height = 2,fig.width = 6, fig.cap = "Checking the model assumptions of the BICModel4"}
par(mar=c(1.8, 4.4, 2, 1.8))
par(mfrow=c(1,2))
...
Using the transformations discussed we first ran an LAR algorithm to obtain coefficients of our variables for different values of the penalty parameter, lambda. The below plot shows a plot of these values against log(lambda).

```{r Cross Validation LASSO, echo = FALSE,out.height='50%',out.width='50%', fig.cap = "Higher lambda gives more desirable model than minimal lambda"} 
par(mfrow = c(1,1))
...
```
We performed cross validation to see how the mean squared error changes with lambda. Both the minimising lambda and the lambda 1se away are shown in the figure above. We chose to use the value of lambda 1se away since using the minimising lambda leads to no parameters being shrunk to zero. A model including every parameter is undesirable, so we instead build our model using the value of lambda 1se above the minimum.  The model we get from this has the coefficients as seen in the findings section.  
...
```{r, echo = FALSE, fig.width=6 , fig.height=2, fig.cap = "Residuals vs Fitted and Scale-Location Plots demonstrate no undesirable patters"}
par(mfrow = c(1,2),mar=c(1.8, 4.4, 1.8, 1.9))
bin <- scale(LASSOresiduals)
bin1 <- which(bin < 0)
bin[bin1] <- -bin[bin1]
plot(LASSOpredicted, sqrt(bin), main = "Scale Location", xlab = "Fitted values", ylab = "sqrt
(Standardised Residuals)",cex=0.5)
plot(LASSOpredicted, LASSOresiduals, main = "Residuals vs Fitted", xlab = "Fitted values", ylab = "Residuals", cex=0.5)
```
...
We investigated ridge regression to help deal with any multicollineaity issues that may arise. Using this form of penalised regression gives us a model with an R squared of 0.484 and a mean squared error of 378.5, which is low since ridge regression trades bias for a reduced variance. Given that the R squared isn't great in comparison to the model made using LASSO we opt to use other penalised regression methods for greater simplicity and explanatory power.
...
crossVal=as.table(matrix(data=c(20.98,16.09,0.4176,19.73,14.72,0.4849),nrow=2,ncol=3,byrow = TRUE))
...
# Creating a linear model with all variables from the current dataset
Model0 <- lm(deathRate ~ Region + incidenceRate + medIncome + povertyPercent + MedianAgeMale + MedianAgeFemale + AvgHouseholdSize + PercentMarried + PctBlack + 
PctMarriedHouseholds + PctPrivateCoverage + PctPublicCoverage + PctEmpPrivCoverage + PctUnemployed16_Over + Edu18_24, data = cancer)
...
# Look at scatter plots and compare weak positive correlations to the correlations shown in original plots.
par(mfrow = c(2,3))
with(cancer5, scatter.smooth(deathRate~Edu18_24, ylab = "deathRate", xlab = "Edu18_24", lpars = list(col = "red", lwd = 2), main = "deathRate against Edu18_24"
,cex.main = 1))
with(cancer5, scatter.smooth(deathRate~PctMarriedHouseholds, ylab = "deathRate", xlab = "PctMarriedHouseholds", lpars = list(col = "red", lwd = 2)
,main = "deathRate against PctMarriedHouseholds", cex.main = 1))
...
model <- train(deathRate~0+incidenceRate+povertyPercent+PctUnemployed16_Over+
  PctPublicCoverage+PctBlack+PercentMarried,data=cancer2,method = "lm", trControl = ctrl)
print(model)
```
...
set.seed(3041)
ridge.fit <- cv.glmnet(model.matrix(Model3), cancer5$deathRate, alpha = 0, nrow = 3041)
predicteddeathRate.ridge <- predict(ridge.fit, s = ridge$lambda.min, newx = model.matrix(Model3))
rsquared.ridge <- (1 - (sum((predicteddeathRate.ridge - cancer5$deathRate)^2)/sum((cancer5$deathRate - mean(cancer5$deathRate))^2)))
rsquared.ridge 
```

## Checking Conformity

```{r conformity, eval = FALSE}
Conformity <- function(x){ #Calculating residuals for one county, using raw dataset with PctEmployed16_Over removed
  return(cancer[x,17]-(387+0.01*boxcox(cancer[x,3],1.5)-23.5*log(cancer[x,4])+2.49*boxcox(cancer[x,10],0.5)-0.212*cancer[x,11]+0.127*cancer[x,13]-8.22*cancer[x,16]
-4.61*(cancer[x,18]=="NorthEast")+3.73*(cancer[x,18]=="South")-12.4*(cancer[x,18]=="West")))#Calculating residuals
}
c(Conformity(282),Conformity(1490),Conformity(2714),Conformity(166),Conformity(2727),Conformity(2682)) #Residuals of points removed in data cleaning
ConformityStates<-function(){ #Calculating average residual across all counties for a single state
  out<-rep(0,51)
  j<-1
  for(i in unique(State)){
    out[j]<-(sum(Conformity(as.numeric(rownames(cancer[State==i,])))))/length(cancer[State==i,1])
    j<-j+1
  }
  return(out)
}
