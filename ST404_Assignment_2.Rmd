...
name=c('intercept','incidenceRate','medIncome','PctUnemploy16_Over','PctPrivateCoverage',
       'PctPublicCoverage','Edu18_24','RegionNortheast','RegionSouth','RegionWest')
beta=c('387','0.010','-23.5','2.49','-0.212','0.127','-8.22','-4.61','3.73','-12.4')
modelTable=as.table(matrix(data=c(beta),nrow=10,ncol=1))
rownames(modelTable)=name
colnames(modelTable)=c('Parameter estimate')
pander::pander(modelTable, caption = "Information on LASSO Model", split.cell = 75, split.table = Inf, style = "grid", justify = c("left","center"), use.hyphening = TRUE)
```
...
Stepwise regression is not favoured over LASSO because LASSO has a better system of penalising results because it has a tuning parameter. Stepwise will often overfit the model to gain a better value of R^2^ and so you get an inflated R^2^ value. This results in a model which is not necesarrily the best at explaining or predicting the data. LASSO also has limitations in that it does not account for multicollinearity but this is also an issue in stepwise regression, so LASSO is a lot better than stepwise regression.
...
We will assess the adequacy of the model by checking if the functional form of the model is correct. We will look at the residuals vs fitted and scale location plots for the untransformed full model below:


``` {r, echo = FALSE, eval = TRUE,fig.height=1.8,fig.width=6}
...
We can see from the plots that there exists heteroscedasticity and non-linearity. We also notice from our initial EDA that some individual predictor variables are not linear and show signs of heteroscedascity, so we will need to perform some transformations of some variables. We do this by using Box-Cox and observations after trialling log transformations. We first transform the individual predictor variables to satisfy homoscedascity, these include the following transformations: $log$(PctBlack$+0.1$), incidenceRate^1.5^, PctUnemployed16_Over^0.5^ and  $log$(medIncome).
...
``` {r checkingassumptions, echo = FALSE, eval = TRUE,out.height='80%',out.width='80%'}
par(mar=c(1.8, 4.4, 2, 1.8))
par(mfrow=c(2,2))
plot(lm(deathRate ~ PctBlack, data = cancer5), 1, cex=0.5,main="log(PctBlack) Residuals Fitted",caption="")
plot(lm(deathRate ~ PctBlack, data = cancer5), 3, cex=0.5,main="log(PctBlack) Scale location",caption="")
plot(lm(deathRate ~ PctUnemployed16_Over, data = cancer5), 1, cex=0.5,main="PctUnemplyed16^0.5 Residuals Fitted",caption="")
plot(lm(deathRate ~ PctUnemployed16_Over, data = cancer5), 3, cex=0.5,main="PctUnemplyed16^0.5 Scale location",caption="")
```

These transformations show evidence of homoscedascity and linearity thus we will update the new model with the transformed variables. The new model presents linearity and the heteroscedascity has been reduced, but it still exists. This can be seen in the residual vs fitted and scale location plots of the full model with these transformations presented below. This could be due to influencial outliers.

``` {r adequacy2, echo = FALSE, eval = TRUE, fig.height=1.8,fig.width=6}
...
```{r Check Assumptions, echo=FALSE,out.height='70%',out.width='70%'}
par(mar=c(1.8, 4.4, 2, 1.8))
par(mfrow=c(2,2))
for (i in (1:4)) {
...
In order to use the functions in the package glmnet with our data we needed to do two things:
1.	Put our data into a matrix
2.	Have the region variable split into three variables (since there are four regions)

Using the transformations rearranging of our data stated above we first run an LAR algorithm to obtain the following coefficents which are plotted against the penalty parameter, lambda. We slso include the value of lambda corresponding to the coeficents minimising the mean squared error. However this lambda shrinks none of the parameters to zero. This model would be very difficult to interpret and so we instead opt to use the value of lambda which is one standard error away. These values of lambda were subject to change when using the cross validation function found in glmnet, so we set a seed of six when building our model in the interest of reproducibility.

```{r Cross Validation LASSO, echo = FALSE,out.height='50%',out.width='50%'} 
par(mfrow = c(1,1))
...
```
The model we get from this has the coefficents as seen in the findings section. 
...
```{r, echo = FALSE, fig.width=9 , fig.height=2.25}
...
``` {r Author's table, echo=FALSE}
crossVal=as.table(matrix(data=c('Initial analysis before variable selection and model comparison','LASSO method section','Initial analysis before variable selection','Stepwise regression section'),nrow=2,ncol=3,byrow = TRUE))
rownames(crossVal)=c('Conor','Daniel','James','Sam')
colnames(crossVal)=c('Contribution')
knitr::kable(crossVal, caption = "Authors' Contribution")
```
