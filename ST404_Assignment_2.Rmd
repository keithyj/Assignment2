...
```{r Cross validation, echo=FALSE}
crossVal=as.table(matrix(data=c(20.98,16.09,0.4176,19.73,NA,0.4849),nrow=1,ncol=3,byrow = TRUE))
...
```{r Define null and full models, echo=FALSE}
...
```{r Cross Validation LASSO, eval = FALSE} 
par(mfrow = c(1,1))
plot(LASSO, "lambda", label = T)
abline(v = log(cvLASSO$lambda.min), col = "blue")
abline(v = log(cvLASSO$lambda.1se), col = "red")
